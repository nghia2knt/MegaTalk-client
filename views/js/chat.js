        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            for (var i in chatsList){
             ...
             document.getElementById('tableRoom').innerHTML += 
             '<tr><td onclick="changeID(\''+chatID+'\')" class="list-group-item list-group-item-action"><h5>'+roomname+'</h5><p class="lastMess-2">'+lastMess+'</p></td></tr>'
            ...
            var chatsListMain = data.data.chatsList.filter(e=> e.roomID == roomChatID)[0];
            ...
            for (var i in messagesList){
                ...
                '<p class="text-start fw-bold">'+messagesList[i].sender+'</p>'+
                '<p class="text-start fw-lighter">'+messagesList[i].createAt+'</p>'+
                '<p class="text-start">'+messagesList[i].content+'</p>'+
                '</td></tr>'                      
            }
...
function sendMess() {
  ...
  '<p class="text-start fw-lighter">'+"Đang gửi tin nhắn"+'</p>'+
  '<p class="text-start">'+content+'</p>'+
  '</td></tr>'     
  $('#myMessageContainer').stop ().animate ({
    scrollTop: $('#myMessageContainer')[0].scrollHeight,
  });
